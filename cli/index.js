...
const SHEET_ID = '1vAcXpFaA1OZQM-5MtryhAHl-KXPYGgSJEdcmhj_2C1M';
...
const getInfoVideo = async (url) => {
    ...
    const API_URL = getURLByIdVideo(idVideo)
    ...
    console.log(`id ${res.aweme_list[0].aweme_id}`)
    console.log(`desc ${res.aweme_list[0].desc}`)
    console.log(`time create ${res.aweme_list[0].create_time}`)
    console.log(`author ${res.aweme_list[0].author.nickname}`)
    const video = res.aweme_list[0].video;
    const statistics = res.aweme_list[0].statistics;
    console.log(`url video có logo ${chalk.green(video.download_addr.url_list[0])}`)
    console.log(`url video không logo ${chalk.green(video.play_addr.url_list[0])}`)
    console.log(`comment_count ${statistics.comment_count}`)
    console.log(`digg_count ${statistics.digg_count}`)
    console.log(`download_count ${statistics.download_count}`)
    console.log(`play_count ${statistics.play_count}`)
    console.log(`share_count ${statistics.share_count}`)
    console.log(`forward_count ${statistics.forward_count}`)
    console.log(`is_ads ${statistics.is_ads}`)
}

const getURLByIdVideo = (idVideo) => {
